...
router.get(`/get-${model}-by-name-subcategories/:id`, category_controller_1.getByNameSubcategories);
router.patch(`/update-${model}/:id`, passport.authenticate('jwt', { session: false }), category_controller_1.update);
router.patch(`/update-${model}-all`, passport.authenticate('jwt', { session: false }), category_controller_1.updateAll);
